...
UpdateNodes.prototype.writeUpdateItems4 = function(writer) {
    ...
    for (var i = 0; i < this.updNodes.length; i++) {
        ...
        if (node.isAgitated) flags |= 0x10; // isAgitated
        if (node.cellType == 3) flags |= 0x20; // isEjected
    ...
    for (var i = 0; i < this.addNodes.length; i++) {
        ...
        if (node.cellType == 3) flags |= 0x20; // isEjected
        writer.writeUInt8(flags >>> 0); // Flags
        if (cellName != null) writer.writeBytes(cellName); // Name
        else writer.writeUInt16(0); // Name
    }
...
UpdateNodes.prototype.writeUpdateItems5 = function(writer) {
    ...
    for (var i = 0; i < this.addNodes.length; i++) {
        ...
        var flags = 0;
        if (node.isSpiked) flags |= 0x01; // isVirus
        if (skinName != null) flags |= 0x04; // isSkinPresent
        if (node.isAgitated) flags |= 0x10; // isAgitated
        if (node.cellType == 3) flags |= 0x20; // isEjected
        writer.writeUInt8(flags >>> 0); // Flags
        if (flags & 0x04) writer.writeBytes(skinName); // Skin Name in UTF8
        if (cellName != null) writer.writeBytes(cellName); // Name
        else writer.writeUInt16(0); // Name
    }
...
UpdateNodes.prototype.writeUpdateItems6 = function(writer) {
    ...
    for (var i = 0; i < this.updNodes.length; i++) {
        ...
        var flags = 0;
        if (node.isSpiked) flags |= 0x01; // isVirus
        if (node.cellType == 0) flags |= 0x02; // isColorPresent (for players only)
        if (node.isAgitated) flags |= 0x10; // isAgitated
        if (node.cellType == 3) flags |= 0x20; // isEjected
        writer.writeUInt8(flags >>> 0); // Flags
        if (flags & 0x02) {
    ...
    for (var i = 0; i < this.addNodes.length; i++) {
        ...
        var flags = 0;
        if (node.isSpiked) flags |= 0x01; // isVirus
        if (true) flags |= 0x02; // isColorPresent (always for added)
        if (skinName != null) flags |= 0x04; // isSkinPresent
        if (cellName != null) flags |= 0x08; // isNamePresent
        if (node.isAgitated) flags |= 0x10; // isAgitated
        if (node.cellType == 3) flags |= 0x20; // isEjected
        writer.writeUInt8(flags >>> 0); // Flags
        if (flags & 0x02) {
        ...
        if (flags & 0x04) writer.writeBytes(skinName); // Skin Name in UTF8
        if (flags & 0x08) writer.writeBytes(cellName); // Cell Name in UTF8
    }
    writer.writeUInt32(0); // Cell Update record terminator
};
// protocol 11
UpdateNodes.prototype.writeUpdateItems11 = function(writer) {
    ...
    for (var i = 0; i < this.updNodes.length; i++) {
        ...
        if (node.isAgitated) flags |= 0x10; // isAgitated
        if (node.cellType == 3) flags |= 0x20; // isEjected
        if (node.cellType == 1) flags |= 0x80; // isFood
        writer.writeUInt8(flags >>> 0); // Flags
...
UpdateNodes.prototype.writeRemoveItems = function(writer, protocol) {
    ...
    if (protocol < 6) writer.writeUInt32(length >>> 0); // RemoveRecordCount
    else writer.writeUInt16(length >>> 0); // RemoveRecordCount
    for (var i = 0; i < this.eatNodes.length; i++) {
        var node = this.eatNodes[i];
        writer.writeUInt32((node.nodeId ^ scrambleId) >>> 0); // Cell ID
    }
