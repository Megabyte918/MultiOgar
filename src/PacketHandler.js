...
PacketHandler.prototype.handshake_onCompleted = function(protocol, key) {
    ...
    if (this.gameServer.config.serverChat == 0) this.gameServer.sendChatMessage(null, this.socket.playerTracker, "This server's chat is disabled.");
    if (this.protocol < 4) this.gameServer.sendChatMessage(null, this.socket.playerTracker, "WARNING: Protocol " + this.protocol + " assumed as 4!");
};
...
PacketHandler.prototype.setNickname = function(text) {
    ...
    if (name.length > this.gameServer.config.playerMaxNickLength) name = name.substring(0, this.gameServer.config.playerMaxNickLength);
...
PacketHandler.prototype.sendPacket = function(packet) {
    ...
    if (!packet || socket.isConnected == null || socket.playerTracker.isMi) return;
