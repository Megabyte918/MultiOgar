...
var fillChar = function(data, char, fieldLength, rTL) {
    ...
    if (rTL === true) {
        ...
        for (var i = result.length; i < fieldLength; i++) result = result.concat(char);
    }
    return result;
};
// Commands
Commands.list = {
    ...
    reset: function(gameServer, split) {
        ...
        while (gameServer.nodes.length) gameServer.removeNode(gameServer.nodes[0]);
        while (gameServer.nodesEjected.length) gameServer.removeNode(gameServer.nodesEjected[0]);
        while (gameServer.nodesFood.length) gameServer.removeNode(gameServer.nodesFood[0]);
        while (gameServer.nodesVirus.length) gameServer.removeNode(gameServer.nodesVirus[0]);
        Commands.list.killall(gameServer, split);
    },
    minion: function(gameServer, split) {
        var id = parseInt(split[1]);
        var add = parseInt(split[2]);
        var name = split.slice(3, split.length)
            .join(' ');
        // Error! ID is NaN
        if (isNaN(id)) {
    ...
    banlist: function(gameServer, split) {
        ...
        for (var i = 0; i < gameServer.ipBanList.length; i += 2) {
            Logger.print(" " + fillChar(gameServer.ipBanList[i], " ", 15) + " | " + (gameServer.ipBanList.length === i + 1 ? "" : gameServer.ipBanList[i + 1]));
        }
    },
    kickbot: function(gameServer, split) {
        ...
        if (!removed) Logger.warn("Cannot find any bots");
        else if (toRemove == removed) Logger.warn("Kicked " + removed + " bots");
        else Logger.warn("Only " + removed + " bots were kicked");
    },
    ...
    kickall: function(gameServer, split) {
        ...
        gameServer.clients.forEach(function(socket) {
            if (socket.isConnected === false) return;
            if (this.id != 0 && socket.playerTracker.pID != this.id) return;
            // remove player cells
            Commands.list.killall(gameServer, split);
            // disconnect
            socket.close(1000, "Kicked from server");
    ...
    mass: function(gameServer, split) {
        ...
        for (var i in gameServer.clients) {
            if (gameServer.clients[i].playerTracker.pID == id) {
                ...
                Logger.print("Set mass of " + getName(client._name) + " to " + (size * size / 100)
                    .toFixed(3));
                break;
            }
        }
    ...
    spawnmass: function(gameServer, split) {
        ...
        for (var i in gameServer.clients) {
            if (gameServer.clients[i].playerTracker.pID == id) {
                ...
                Logger.print("Set spawnmass of " + getName(client._name) + " to " + (size * size / 100)
                    .toFixed(3));
            }
        }
        if (client == null) return void Logger.warn("That player ID is non-existant!");
        ...
        for (var i in gameServer.clients) {
            if (gameServer.clients[i].playerTracker.pID == id) {
                ...
                Entity.PlayerCell.prototype.getSpeed = function(dist) {
                    ...
                    speed = this.owner.customspeed ? speed * 40 * this.owner.customspeed : // Set by command
    ...
    name: function(gameServer, split) {
        ...
        var name = split.slice(2, split.length)
            .join(' ');
    ...
    unban: function(gameServer, split) {
        if (split.length < 2 || !split[1] || split[1].trim()
            .length < 1) {
    ...
    playerlist: function(gameServer, split) {
        ...
        for (var i = 0; i < sockets.length; i++) {
            ...
            if (socket.closeReason != null) {
                ...
                position = fillChar(getPos(client)
                    .x >> 0, ' ', 5, true) + ', ' + fillChar(getPos(client)
                    .y >> 0, ' ', 5, true);
    ...
    status: function(gameServer, split) {
        ...
        var scores = [];
        for (var i in gameServer.clients) scores.push(getScore(gameServer.clients[i].playerTracker))
        ...
        Logger.print("Average score: " + (scores.reduce((x, y) => x + y) / scores.length)
            .toFixed(2));
        Logger.print("Server has been running for " + Math.floor(process.uptime() / 60) + " minutes");
        Logger.print("Current memory usage: " + Math.round(process.memoryUsage()
    ...
    calc: function(gameServer, split) {
        ...
        else Logger.print("The specified mass is " + (Math.sqrt(num * 100))
            .toFixed(2) + " in size");
    },
